...
static int DIM_elapsed_time = 0, New_elapsed_time = 0;
...
void (*user_routine)();
{
	...
	if(Alarm_runs)
	{
		if((curr_time > time) || (queue_id == SPECIAL_QUEUE))
	    {

			curr_time -= my_alarm(0);
#ifdef LYNXOS
#ifdef RAID
			--curr_time ;
#endif
...
TIMR_ENT *entry;
{
	...
	entry->time_left = entry->time + curr_time;
	ENABLE_AST
	return(time_left);
}
